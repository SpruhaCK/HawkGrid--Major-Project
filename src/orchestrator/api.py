...
#             # Trigger Hive Mind for Critical/Extreme scores (4 or 5)
...
#                 )
#             else:
#                 # Standard block for lower severity
...
def resolve_asset(public_ip: str):
    if public_ip in IP_MAPPING_CACHE:
        return IP_MAPPING_CACHE[public_ip]
    return {"private_ip": public_ip, "provider": None}
...
async def lifespan(app: FastAPI):
    ...
    # 🚨 NEW: DYNAMIC PRESENTER WHITELISTING
    try:
...
def detect_anomaly(payload: LogFeatures):
    try:
        ...
        incident_data["anomaly_score"] = detection.get("anomaly_score", 0.0)
        incident_data["attack_type"] = detection.get("attack_type", "NORMAL")
        incident_data["owasp_risk_score"] = detection.get("owasp_risk_score", 1)
        incident_data["severity"] = detection.get("severity", "LOW")
        incident_data["raw_event"] = payload.model_dump()
        ...
        if detection.get("is_anomaly") and incident_data["attack_type"] != "NORMAL" and provider:
            ...
            # Trigger Hive Mind for Critical scores (4 or 5)
            if risk_score >= 4:
                # 🚨 Pass the dynamically discovered IP
                safe_ip = getattr(app.state, "whitelisted_ip", "127.0.0.1")
                
                response_action = execute_cross_cloud_quarantine(
                    incident_data=incident_data,
                    target_provider_name=provider_name,
                    all_providers=all_connected_clouds,
                    whitelisted_ip=safe_ip 
                )
            else:
                response_action = execute_standard_block(
                    incident_data=incident_data,
